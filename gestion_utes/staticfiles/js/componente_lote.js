document.addEventListener("DOMContentLoaded", () => {
  ...
  guardarTodosBtn.addEventListener("click", async () => {
    ...
    // Capturamos las características generales (del DOM principal)
    const caracteristicas = Array.from(document.querySelectorAll("#characteristicsContainer input"));

    for (const form of formularios) {
      const formData = new FormData(form);

      // Agregamos las características generales al formData
    ...
    if (errores === 0) {
      alert("✅ Todos los componentes fueron registrados.");
      window.location.href = URL_LISTA_COMPONENTES;
    } else {
      alert(`⚠️ Se registraron con errores ${errores} componentes.`);
    }
  });
});
